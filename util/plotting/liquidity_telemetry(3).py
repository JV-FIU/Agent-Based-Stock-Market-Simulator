...
'''def make_plots1(plot_inputs, plot_params_dict, ticker, date1, title=None, out_file="liquidity_telemetry.png"):
...
def make_plots1(plot_inputs, plot_params_dict, ob_path, ticker, date1, title=None, out_file="liquidity_telemetry.png"):
    """ Produce a plot with three subplots:
          1. Mid-price over time.
          2. Liquidity dropout events over time
          3. Transacted volume over time
    ...
    if plot_inputs['fundamental'] is not None:
        plot_inputs['fundamental'].loc[xmin:xmax].plot(color='blue', label="Fundamental")
    plot_inputs['mid_price'].loc[xmin:xmax].plot(color='black', label="Price")
    plt.axvspan(shade_start, shade_end, alpha=0.2, color='grey')
    plt.gca().xaxis.set_major_formatter(mdates.DateFormatter("%H:%M"))
    plt.gca().xaxis.set_minor_formatter(mdates.DateFormatter("%H:%M"))
    plt.gca().xaxis.set_visible(True)
    ...
    plt.subplots_adjust(hspace=0.05)
    plt.savefig('simulated_values.png', format='png', dpi=300, transparent=False, bbox_inches='tight', pad_inches=0.03)


def make_plots0(plot_inputs, plot_params_dict, ob_path, ticker, date1, title=None, out_file="liquidity_telemetry.png"):
    import pandas as pd
    ...
    xmin = pd.to_datetime(date2 + " 09:30:00")
    xmax = pd.to_datetime(date2 + " 16:00:00")
...
# NEW
def comparison(plot_inputs, plot_params_dict, ob_path, ticker, date1, title=None, out_file="liquidity_telemetry.png"):
    import yfinance as yf
...
def main(exchange_path, ob_path, ticker, date1, title=None, outfile='liquidity_telemetry.png', verbose=False):
    ...
    print("Plotting...")
    make_plots1(plot_inputs, PLOT_PARAMS_DICT, ob_path, ticker, date1, title=title, out_file=outfile)
    make_plots0(plot_inputs, PLOT_PARAMS_DICT, ob_path, ticker, date1, title=title, out_file=outfile)
    ...
    comparison(plot_inputs, PLOT_PARAMS_DICT, ob_path, ticker, date1, title=title, out_file=outfile)
...
if __name__ == '__main__':
    ...
    main(stream, book, ticker, date1, title=title, outfile=out_filepath, verbose=verbose)
